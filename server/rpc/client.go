...
func (c *Client) HelloRPC() (map[string]string, error) {
...
func (c *Client) dispatch(conn net.Conn) {
    ...
    for {
        ...
        case err := <-c.readErr:
            fmt.Println("<-readErr", "err", err)
            c.closeRequestOps(err)
            conn.Close()
            reading = false

        case newconn := <-c.reconnected:
            fmt.Println("<-reconnected", "reading", reading, "remote", conn.RemoteAddr())
            if reading {
...
func (c *Client) handleNotification(msg *jsonrpcMessage) {
    if !strings.HasSuffix(msg.Method, notificationMethodSuffix) {
        fmt.Println("dropping non-subscription message", "msg", msg)
        return
    }
    ...
    if err := json.Unmarshal(msg.Params, &subResult); err != nil {
        fmt.Println("dropping invalid subscription message", "msg", msg)
        return
    }
...
func (c *Client) handleResponse(msg *jsonrpcMessage) {
    ...
    if op == nil {
        fmt.Println("unsolicited response", "msg", msg)
        return
    }
